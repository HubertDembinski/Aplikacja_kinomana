...
<html>
  ...
  <body>
    ...
    <form name = "formularz-login" class = "formularz" method="post" action="">
      <input type="text" class="szukajstyle" name="nazwafilmpost" placeholder="Wyszukaj po nazwie">
      <input type="submit" class = "submitstyle" value="SZUKAJ">
    </form>
    </div>
           <?php
        if($_SERVER["REQUEST_METHOD"] == "POST")
        {
          $nazwafilmpost=$_POST['nazwafilmpost'];
          $result2 = mysqli_query($connection, "SELECT idf, nazwa_film, miniatura, kategorie.nazwa, rok, rezyser, plik FROM filmy INNER JOIN kategorie ON filmy.idk = kategorie.idk WHERE filmy.nazwa_film = '$nazwafilmpost';") or die ("DB error: $dbname");
